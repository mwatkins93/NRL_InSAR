...
insar_3724 <- insar_tidy %>%
  filter(id %in% "7583724")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_3724, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7583724")
...
### InSAR 3724
insar3724_plot <- insar_long %>% 
  filter(point_id %in% "7583724") %>% 
...
lansdowne_hist / attawa_hist_q / insar3724_plot + plot_layout(guides = "collect")
