...
insar_0186 <- insar_tidy %>%
  filter(id %in% "7580186")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_0186, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7580186")
...
### InSAR 0186
insar0186_plot <- insar_long %>% 
  filter(point_id %in% "7580186") %>% 
...
lansdowne_hist / attawa_hist_q / insar0186_plot + plot_layout(guides = "collect")
