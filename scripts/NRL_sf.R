...
insar_7571 <- insar_tidy %>%
  filter(id %in% "7537571")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_7571, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7537571")
...
attawa_hist_q <- attawapiskat_exp %>%
  ...
  # Annotations
  geom_vline(data = insar_7571, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  geom_text(data = insar_7571,
            mapping = aes(x = date - 17, y = 1600, label = date,),
            inherit.aes = FALSE,
            size = 3,
            hjust = 1,
            angle = 90) + 
...
### InSAR 7571
insar7571_plot <- insar_long %>% 
  filter(point_id %in% "7537571") %>% 
...
lansdowne_hist / attawa_hist_q / insar7571_plot + plot_layout(guides = "collect")
