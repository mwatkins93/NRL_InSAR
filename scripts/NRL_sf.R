...
insar_3726 <- insar_tidy %>%
  filter(id %in% "7583726")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_3726, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7583726")
...
### InSAR 3726
insar3726_plot <- insar_long %>% 
  filter(point_id %in% "7583726") %>% 
...
lansdowne_hist / attawa_hist_q / insar3726_plot + plot_layout(guides = "collect")
