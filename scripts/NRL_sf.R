...
insar_2419 <- insar_tidy %>%
  filter(id %in% "7562419")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_2419, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7562419")
...
### InSAR 2419
insar2419_plot <- insar_long %>% 
  filter(point_id %in% "7562419") %>% 
...
lansdowne_hist / attawa_hist_q / insar2419_plot + plot_layout(guides = "collect")
