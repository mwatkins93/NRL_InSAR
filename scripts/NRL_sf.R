...
insar_2413 <- insar_tidy %>%
  filter(id %in% "7562413")
...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_2413, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 7562413")
...
### InSAR 2413
insar2413_plot <- insar_long %>% 
  filter(point_id %in% "7562413") %>% 
...
lansdowne_hist / attawa_hist_q / insar2413_plot + plot_layout(guides = "collect")
