...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar_1b, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 4997256")
...
### InSAR 1A
insar1b_plot <- insar_long %>% 
  filter(point_id %in% "4997256") %>% 
...
lansdowne_hist / attawa_hist_q / insar1b_plot + plot_layout(guides = "collect")
