...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar2, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 5088845")
...
### InSAR 1A
insar2_plot <- insar_long %>% 
  filter(point_id %in% "5088845") %>% 
...
lansdowne_hist / attawa_hist_q / insar2_plot + plot_layout(guides = "collect")
