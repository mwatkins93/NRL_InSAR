...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar9, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 4912985")
...
### InSAR 1A
insar9_plot <- insar_long %>% 
  filter(point_id %in% "4912985") %>% 
...
lansdowne_hist / attawa_hist_q / insar9_plot + plot_layout(guides = "collect")
