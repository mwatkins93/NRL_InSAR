...
lansdowne_hist <- lansdowne_clean %>% 
  ...
  # Annotations
  geom_vline(data = insar3, aes(xintercept = date, colour = "salmon"), linetype = "dashed") +
  ...
  ggtitle("Historical Attawapiskat River discharge | Weather station: Lansdowne House (AUT) | InSAR ID: 5050228")
...
### InSAR 1A
insar3_plot <- insar_long %>% 
  filter(point_id %in% "5050228") %>% 
...
lansdowne_hist / attawa_hist_q / insar3_plot + plot_layout(guides = "collect")
